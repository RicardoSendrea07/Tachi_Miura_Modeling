...
% hfssIncludePaths('C:\Program Files\ANSYS Inc\v190\ansys\bin\winx64\ANSYS190.exe');
...
Dir = pwd;
tmpPrjFile    = [Dir,'\HFSSFiles' '\tmpDipole.aedt'];
tmpDataFile   = [Dir, '\tmpData.m'];
hfssScriptFile = [Dir, '\dipole_example.vbs'];
...
%     
    ...
    figure(1);
    hold on; grid on;
%     plot(fC/1e6, 20*log10(abs(S)), pltCols(mod(iIters, nCols) + 1)); 
    hold on;
