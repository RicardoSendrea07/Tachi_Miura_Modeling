...
Transformation = RMatrix1.doATransformation(0,0,0,Points);
...
%     
%     cP = [mean(P(1,:)); mean(P(2,:)); mean(P(3,:))]; %to check if centroid
%     %are now aligned
%     cQ = [mean(Q(1,:)); mean(Q(2,:)); mean(Q(3,:))];
    ...
    T2 = Transformation - (RMatrix2 * Points);
    T = Transformation - (RMatrix * Points);
    
    Check = (RMatrix * Points) + T; % Should be equal to Transformation
    Check2 = (RMatrix2 * Points) + T2; % Should be equal to Transformation
